...
export default function StreamingStoryCreation({
      ...
      progressIntervalRef.current = setInterval(() => {
        setProgress((prev) => {
          ...
          if (diff > 0) {
            ...
            console.log(
              `📊 Progress: ${prev.toFixed(1)}% → ${newProgress.toFixed(
                1
  ...
  useEffect(() => {
    if (progress >= 99.5 && storyIdRef.current) {
      console.log(
        "✅ [CLIENT] Progress at 100%, redirecting to story:",
        storyIdRef.current
      );
      setTimeout(() => {
  ...
  useEffect(() => {
    ...
    console.log(
      "🎧 [CLIENT] useEffect running, hasStarted:",
    ...
    const handleTextChunk = (data: { chunk: string; fullContent: string }) => {
      ...
      if (stage === "outline") {
        ...
        const charsProgress = Math.min(
          15,
          (currentChars / estimatedTotalChars) * 15
        );
        const newTarget = Math.min(25, 10 + charsProgress);

        targetProgressRef.current = newTarget;
        console.log(
          `📊 [STREAMING] ${currentChars} chars → ${newTarget.toFixed(
            1
          )}% progress`
        );
      }
    ...
    const handleGenerationUpdate = (data: any) => {
      try {
        console.log(
          "📡 [CLIENT] Generation update received:",
          JSON.stringify(data)
        );
        ...
        switch (data.stage) {
          case "outline_complete":
            console.log(
              "📡 [CLIENT] Outline complete, resetting streaming metrics"
            );
          ...
          case "scenes_parsed":
            console.log(
              "📡 [CLIENT] Initializing scenes array with",
              data.sceneCount,
              "scenes"
            );
          ...
          case "scene_streaming":
            console.log(
              "📡 [CLIENT] Scene streaming update for scene",
              data.sceneIdx
            );
            ...
            if (totalScenesRef.current > 0) {
              ...
              const currentSceneProgress =
                baseProgress + data.sceneIdx * progressPerScene;
              targetProgressRef.current = Math.min(90, currentSceneProgress);
              console.log(
                `📊 [CLIENT] Scene ${data.sceneIdx + 1}/${
                  totalScenesRef.current
                } - Target progress: ${targetProgressRef.current}%`
              );
            }
          ...
          case "image_generating":
            console.log(
              "📡 [CLIENT] Image generating for scene",
              data.sceneIdx
            );
            setScenes((prev) => {
              const updated = [...prev];
    ...
    const handleSuccess = (data: { storyId: string; story: any }) => {
      ...
      setTimeout(() => {
        if (storyIdRef.current && progress < 99.5) {
          console.log(
            "⚠️ [CLIENT] Timeout reached, forcing redirect at",
            progress.toFixed(1) + "%"
          );
    ...
    if (!hasStartedRef.current) {
      console.log(
        "📤 [CLIENT] Emitting create-story-streaming event to server..."
      );
      setStage("outline");
    ...
    <div className="w-full">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl p-8 mb-6 border border-purple-100">
          ...
          {/* Current Stage Info */}
          <div
            className={`${colorClasses.bg} rounded-xl p-4 border ${colorClasses.text} border-current/20`}
          >
            ...
            {message && message !== currentStageInfo.description && (
              <div className="text-sm pl-13 opacity-80 mt-1">{message}</div>
            )}
          ...
          <div className="mt-6 flex items-center justify-between text-xs">
            {[
              ...
              const isCurrent =
                progress >= (stageProgress[idx - 1] || 0) &&
                progress < stageProgress[idx];
                  ...
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center mb-1 transition-all ${
                      isComplete
                        ...
                        : isCurrent
                        ? `${colorClasses.bg} ${colorClasses.text} ring-2 ${colorClasses.ring} scale-110`
            ...
            {scenes.map((scene, idx) => (
              <div
                ...
                className={`bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl overflow-hidden transition-all duration-500 border ${
                  idx === currentSceneIdx
                    ? "ring-4 ring-purple-400 border-purple-200 scale-[1.02]"
                    : "border-gray-100"
                  ...
                  <div className="h-[400px] bg-gradient-to-br from-purple-50 via-pink-50 to-orange-50 flex items-center justify-center relative overflow-hidden">
                    ...
                    <div className="text-center relative z-10">
                      ...
                      <p className="text-gray-500 text-sm">
                        Creating stunning visuals for this scene
                      </p>
                    </div>
                  </div>
                ) : scene.content ? (
