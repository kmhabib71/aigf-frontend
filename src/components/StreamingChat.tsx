...
interface StreamingChatProps {
  ...
  onStart: () => void;
  onComplete: (imageUrls?: string[], imageMetadata?: any, textContent?: string) => void;
  onToolProcessing: (message: string) => void;
}

interface StreamingResponse {
  type: 'chunk' | 'complete' | 'error' | 'tool_processing' | 'follow_up_start' | 'fallback_to_http' | 'processing_started';
  content?: string;
  fullContent?: string;
  error?: string;
  message?: string;
...
export const StreamingChat = React.forwardRef<any, StreamingChatProps>(({
  ...
  useEffect(() => {
    ...
    const handleStreamingResponse = (data: StreamingResponse) => {
      ...
      if (conversationId && data.conversationId !== conversationId) {
        console.log(`🔍 Filtering out response for ${data.conversationId}, expecting ${conversationId}`);
        return;
      }

      console.log(`✅ Processing streaming response for conversation ${data.conversationId}`);

      switch (data.type) {
        case 'chunk':
          // Real-time streaming chunk - pass incremental chunk to parent
          const incrementalContent = data.content || '';
          console.log(`📨 Received chunk (${incrementalContent.length} chars):`, incrementalContent.substring(0, 50));
        ...
        case 'complete':
          ...
          if (data.imageUrls && data.imageUrls.length > 0) {
            ...
            console.log(`🖼️ First image preview (50 chars):`, data.imageUrls[0].substring(0, 50));
          }
          ...
          if (data.content && currentStreamingContent === "") {
            console.log(`📝 Sending non-streaming content: "${data.content}"`);
  ...
  }, [socket, conversationId, onResponse, onError, onComplete, onToolProcessing]);
