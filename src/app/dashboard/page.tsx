...
export default function DashboardPage() {
  ...
  // Wrap search param usage in Suspense as required by Next.js
  function PaymentWatcher() {
    const sp = useSearchParams();
    useEffect(() => {
      if (!isAuthenticated) return;
      const p = sp?.get('payment');
      const targetPlan = sp?.get('plan') || undefined;
      if (p === 'success') {
    ...
    }, [isAuthenticated, refreshUserProfile, polling, router, userProfile?.plan]);
    return null;
  }
  ...
  return (
    ...
    >
      ...
      <div className="relative z-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 pt-24">
        ...
        {/* Search params watcher (wrapped in Suspense) */}
        <Suspense fallback={null}>
          <PaymentWatcher />
        </Suspense>

        {/* Show activation banner if needed */}
        {showPaymentBanner.status !== 'none' && (
          <div className={`mb-4 rounded-xl border px-4 py-3 ${showPaymentBanner.status === 'pending' ? 'bg-amber-50 border-amber-200 text-amber-900' : 'bg-emerald-50 border-emerald-200 text-emerald-900'}`}>
            {showPaymentBanner.status === 'pending' ? (
              <div className="flex items-center justify-between">
