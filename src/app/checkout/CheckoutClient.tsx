...
export default function CheckoutClient({
  ...
  useEffect(() => {
    if (isAuthenticated === false && plan) {
      const params = new URLSearchParams({ plan });
      // Preserve selected plan and redirect back after login
      router.replace(`/login?redirect=/checkout?${params.toString()}`);
    }
      ...
      <div className="relative z-20 max-w-5xl mx-auto px-4 py-20 pt-28">
        <div className="mb-10 text-center">
          ...
          <p className="mt-2 text-white/90">
            Select crypto now. Cards are coming soon.
          </p>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="bg-white/10 backdrop-blur-md rounded-2xl border border-white/20 p-6 text-white">
            ...
            <p className="text-sm text-white/80 mt-2">
              Lower network fees on TRC20/BEP20 where available. ERC20 also
              works.
            </p>
            <div className="mt-5 grid grid-cols-1 gap-3">
                ...
                onClick={async () => {
                  try {
                    ...
                    const resp = await fetch(
                      `${backendUrl}/api/payment/nowpayments/create`,
                      {
                        method: "POST",
                        headers: {
              ...
              >
                {creating
                  ? "Preparing..."
                  : `Continue - $${selectedPlan.price}/month`}
              </button>
              {nowUrl && (
                <button
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-2xl border border-white/20 p-6 text-white">
            ...
            <p className="text-sm text-white/80 mt-2" suppressHydrationWarning>
              <span className="bg-cyan-500 text-gray-900 py-1 px-2 rounded-lg">
                TailoredPay
              </span>{" "}
            ...
            <button
              disabled
              className="mt-5 w-full py-3 rounded-xl bg-white/10 text-white font-bold border border-white/20 cursor-not-allowed"
            >
              Coming Soon
            </button>
          </div>
